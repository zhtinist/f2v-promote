...
var dispatcherSvc *service.PromoteDispatcherService
...
func initService() {
	...
	dispatcherSvc = service.NewPromoteDispatcherService(
		promoteLogRepo,
		mnsClient,
		cfg,
	)
...
func HandleRequest(ctx context.Context, event events.Data) (string, error) {
	...
	return dispatcherSvc.Run(), nil
}

func main() {
	if os.Getenv("FC_RUNTIME_API") == "" {
		fmt.Println("本地运行模式，开始 Dispatcher 推送...")
		initService()
		for {
			result := dispatcherSvc.Run()
			fmt.Println(result)
